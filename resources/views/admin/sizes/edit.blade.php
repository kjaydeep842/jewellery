...
<h1 class="text-3xl font-premium font-bold mb-6 text-zinc-900">Edit Size</h1>
...
<div class="bg-white p-8 rounded-xl shadow-lg border border-zinc-100 animate-enter max-w-2xl">
    ...
    <form action="{{ route('admin.sizes.update', $size->id) }}" method="POST">
        ...
        <div class="mb-4">
            <label class="font-bold text-zinc-700 mb-2 block font-heading">Size Number <span class="text-red-500">*</span></label>
            <input type="text" name="number" value="{{ $size->number }}"
        ...
        <div class="mb-4">
            ...
            <input type="number" name="sort_order" value="{{ $size->sort_order }}"
                class="w-full border-zinc-300 rounded-lg shadow-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-shadow p-2.5">
        </div>
        ...
        <div class="mb-6">
            <label class="inline-flex items-center cursor-pointer">
                ...
                <input type="checkbox" name="status" value="1" class="sr-only peer" {{ $size->status ? 'checked' : '' }}>
