...
@section('content')
    ...
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-zinc-800">Orders</h1>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-zinc-50 border-b border-zinc-100">
                        <th class="p-4 text-xs font-bold uppercase text-zinc-500 tracking-wider">Customer</th>
                        <th class="p-4 text-xs font-bold uppercase text-zinc-500 tracking-wider">Email</th>
                        <th class="p-4 text-xs font-bold uppercase text-zinc-500 tracking-wider">Amount</th>
                ...
                <tbody class="divide-y divide-zinc-100">
                    @forelse ($orders as $order)
                        <tr class="hover:bg-zinc-50/50 transition-colors">
                            <td class="p-4 font-medium text-zinc-700">{{ $order->customer_name }}</td>
                            <td class="p-4 text-zinc-500">{{ $order->email }}</td>
                            <td class="p-4 font-bold text-amber-600">₹{{ number_format($order->total_amount, 2) }}</td>
                            <td class="p-4">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                            @if(strtolower($order->status) == 'completed') bg-emerald-100 text-emerald-800 border border-emerald-200
                                            @elseif(strtolower($order->status) == 'pending') bg-amber-100 text-amber-800 border border-amber-200
                                            @elseif(strtolower($order->status) == 'cancelled') bg-rose-100 text-rose-800 border border-rose-200
                                            @else bg-zinc-100 text-zinc-800 border border-zinc-200 @endif">
                            ...
                            <td class="p-4">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <form action="{{ route('admin.orders.destroy', $order->id) }}" method="POST"
                                        class="inline-block" onsubmit="return confirm('Delete this order?');">
                                        @csrf
                                        @method('DELETE')
                                        <button
