...
<div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 animate-fade-in-down">
    ...
    <a href="{{ route('admin.our_stories.create') }}"
...
<div class="bg-white rounded-2xl shadow-xl border border-zinc-100 overflow-hidden animate-fade-in-up"
    ...
    <div class="overflow-x-auto">
        <table id="storiesTable" class="w-full text-left border-collapse">
            <thead class="bg-zinc-50/50 border-b border-zinc-100">
                <tr>
                    ...
                    <th class="p-5 font-bold font-heading text-xs uppercase tracking-wider text-zinc-500">Image</th>
            ...
            <tbody class="divide-y divide-zinc-50">
                ...
                <tr class="group hover:bg-amber-50/30 transition-colors duration-200">
                    <td class="p-5 text-zinc-500 font-mono text-sm">#{{ $story->id }}</td>
                    <td class="p-5">
                        <span class="px-2 py-1 rounded-md text-xs font-semibold
                            {{ $story->type === 'feature' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700' }}">
                            {{ ucfirst($story->type) }}
                        </span>
                    </td>
                    <td class="p-5 font-medium text-zinc-800">{{ $story->title ?? 'N/A' }}</td>
                    <td class="p-5">
                        @if($story->image)
                        <div class="h-12 w-12 rounded-lg overflow-hidden border border-zinc-200 shadow-sm relative group/img bg-white p-1">
                        ...
                        @else
                        <span class="text-zinc-400 text-xs italic px-2 py-1 bg-zinc-100 rounded">No image</span>
                        @endif
                    </td>
                    <td class="p-5 text-zinc-600 text-sm max-w-xs truncate">{{ Str::limit($story->description, 50) }}</td>
                    ...
                    <td class="p-5 text-right">
                        <div class="flex items-center justify-end space-x-2">
                            <a href="{{ route('admin.our_stories.edit', $story->id) }}"
                            ...
                            <form action="{{ route('admin.our_stories.destroy', $story->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this content?');">
                                @csrf
                                @method('DELETE')
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('change', '.toggle-status', function() {
            ...
            fetch(`/admin/our_stories/${id}/toggle`, {
                    method: "PATCH",
                    headers: {
