...
@section('title', 'Edit Metal Color')

@section('content')

<h1 class="text-3xl font-premium font-bold mb-6 text-zinc-900">Edit Metal Color</h1>

<div class="bg-white p-8 rounded-xl shadow-lg border border-zinc-100 animate-enter max-w-2xl">

    <form action="{{ route('admin.metal_colors.update', $metal_color->id) }}" method="POST">
        @csrf
        @method('PUT')

        <div class="mb-4">
            <label class="font-bold text-zinc-700 mb-2 block font-heading">Metal Name <span class="text-red-500">*</span></label>
            <input type="text" name="name" value="{{ $metal_color->name }}"
        ...
        <div class="mb-4">
            ...
            <div class="flex items-center space-x-3">
                <input type="color" id="colorPicker" value="{{ $metal_color->color_code }}" class="w-12 h-10 p-1 rounded cursor-pointer border border-gray-300" onchange="document.getElementById('colorCode').value = this.value">
                <input type="text" id="colorCode" name="color_code" value="{{ $metal_color->color_code }}"
                    class="w-full border-zinc-300 rounded-lg shadow-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-shadow p-2.5"
                    required
        ...
        <div class="mb-4">
            ...
            <input type="number" name="sort_order" value="{{ $metal_color->sort_order }}"
                class="w-full border-zinc-300 rounded-lg shadow-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-shadow p-2.5">
        </div>
        ...
        <div class="mb-6">
            <label class="inline-flex items-center cursor-pointer">
                ...
                <input type="checkbox" name="status" value="1" class="sr-only peer" {{ $metal_color->status ? 'checked' : '' }}>
