...
@section('title', 'Admin - Edit Legal Page')
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in-up">
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-zinc-100 overflow-hidden">
        <div class="p-8">
            <form action="{{ route('admin.legal-pages.update', $legalPage->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="type" id="type" required
                            ...
                            <option value="terms" {{ old('type', $legalPage->type) == 'terms' ? 'selected' : '' }}>Terms & Conditions</option>
                ...
                <div>
                    ...
                    <textarea name="content" id="content_editor" rows="15"
                        class="w-full px-4 py-3 rounded-lg border border-zinc-300 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-shadow shadow-sm placeholder-zinc-400"
                        placeholder="Enter the legal content here...">{{ old('content', $legalPage->content) }}</textarea>
                </div>

                <div class="flex items-center">
                    <label class="relative inline-flex items-center cursor-pointer group">
                        <input type="checkbox" name="status" value="1" class="sr-only peer" {{ $legalPage->status ? 'checked' : '' }}>
