...
@section('content')
    ...
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-zinc-800">Tags</h1>
                ...
                <tbody class="divide-y divide-zinc-100">
                    @foreach($tags as $tag)
                        <tr class="hover:bg-zinc-50/50 transition-colors">
                            <td class="p-4 font-medium text-zinc-700">{{ $tag->name }}</td>
                            <td class="p-4">
                                @if(strtolower($tag->status) == 'active')
                                    <span
                                ...
                                @else
                                    <span
                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-zinc-100 text-zinc-800 border border-zinc-200">
                                        {{ ucfirst($tag->status) }}
                                    </span>
                                @endif
                            </td>
