        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            thumbnails.forEach(function (thumbnail) {
                thumbnail.addEventListener('click', function () {
                    ...
                    // Swap: Set clicked thumbnail's image as main
                    ...
                    // Update the data attribute for future swaps
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[1.5fr_1fr] gap-6 sm:gap-8 md:gap-10 xl:gap-12">
                ...
                <div class="space-y-2 sm:space-y-3 md:space-y-4" style="border-radius: 0;">
                    ...
                    <div class="w-full max-w-full lg:max-w-[1143px] mx-auto" style="border-radius: 0;">
                        <div id="image-zoom-container" class="relative w-full bg-white overflow-hidden cursor-zoom-in" style="aspect-ratio: 1143 / 1319; border-radius: 0;" onmousemove="zoomImageHover(event)" onmouseleave="resetZoomHover()">
                            @php
                                $pMainImagePath = $product->image ?: ($product->images->first() ? $product->images->first()->image_path : null);
                            @endphp
                            @if($pMainImagePath)
                                <img id="main-image" src="{{ asset('storage/' . $pMainImagePath) }}"
                                    alt="{{ $product->name }}"
                            ...
                            @else
                                <div class="absolute inset-0 w-full h-full flex items-center justify-center bg-gray-50 text-gray-400 pointer-events-none">
                                    No Image
                                </div>
                            @endif
                        </div>
                            ...
                            <div class="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-5 lg:grid-cols-5 gap-[10px]">
                                @foreach($product->images as $image)
                                    <div class="thumbnail-item cursor-pointer bg-white transition-all duration-200 hover:border-2 hover:border-[#CBA65A] rounded-lg"
                                        style="aspect-ratio: 1 / 1;" data-image-src="{{ asset('storage/' . $image->image_path) }}">
                ...
                <div id="content-about" class="bg-[#FAF8F1] w-full max-w-[1120px] mx-auto transition-opacity duration-300">
                    <div
                        ...
                        <div
                            ...
                            @endphp
                            @if($aMainImagePath)
                                <img src="{{ asset('storage/' . $aMainImagePath) }}"
                            ...
                            <div class="flex flex-col gap-3">
                                <div
                                    ...
                                    <a href="{{ route('product.details', $related->slug) }}"
                                        ...
                                        @endphp
                                        @if($rMainImagePath)
                                            <img src="{{ asset('storage/' . $rMainImagePath) }}"
                                                alt="{{ $related->name }}"
