...
<div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
    <h1 class="text-2xl sm:text-3xl font-premium font-bold text-zinc-900 tracking-wide">Sizes</h1>
...
<div class="bg-white border border-zinc-100 rounded-xl shadow-lg shadow-zinc-200/50 overflow-x-auto animate-enter p-4">
    <table id="sizesTable" class="w-full text-left border-collapse stripe hover">
        <thead class="bg-zinc-50 text-zinc-900 border-b border-zinc-200">
            <tr>
                ...
                <th class="p-4 font-bold font-heading text-sm uppercase tracking-wider">Number</th>
        ...
        <tbody class="divide-y divide-zinc-100 text-sm">
            ...
            <tr class="group hover:bg-amber-50/50 transition-colors">
                <td class="p-4 text-zinc-500">#{{ $size->id }}</td>
                <td class="p-4 font-bold text-zinc-800">{{ $size->number }}</td>
                <td class="p-4 text-zinc-600">{{ $size->sort_order }}</td>
                <td class="p-4">
                ...
                <td class="p-4">
                    <div class="flex items-center space-x-2">
                        ...
                        <form action="{{ route('admin.sizes.destroy', $size->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure?');">
                            @csrf
                            @method('DELETE')
