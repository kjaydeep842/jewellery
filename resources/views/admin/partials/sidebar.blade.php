<aside class="w-64 bg-zinc-950 text-amber-50 shadow-2xl h-screen sticky top-0 flex flex-col border-r border-zinc-800">

    {{-- TOP SECTION --}}
    <div class="h-20 flex items-center justify-center border-b border-zinc-800 bg-black">
        <div class="flex items-center space-x-3 px-4">
            <!-- Logo Image -->
            <img src="{{ asset('img/logo.png') }}" alt="Tattsvi Gems"
            ...
            <h2
                ...
                Tattsvi<span class="font-light text-amber-100"></span>
            </h2>
        </div>
    </div>
    ...
    <nav class="flex-1 px-3 py-6 space-y-2 overflow-y-auto">
        ...
        @foreach ($menu as $item)
            <a href="{{ route($item['route']) }}"
                class="group flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-300
                    {{ request()->routeIs($item['route'])
            ? 'bg-gradient-to-r from-amber-600 to-yellow-500 text-black shadow-[0_0_15px_rgba(234,179,8,0.4)] border border-yellow-400/50'
