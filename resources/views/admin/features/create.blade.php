...
@section('title', 'Admin - Add Feature')
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in-up">
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-zinc-100 overflow-hidden">
        <div class="p-8">
            <form action="{{ route('admin.features.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3" required
                        class="w-full px-4 py-3 rounded-lg border border-zinc-300 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-shadow shadow-sm placeholder-zinc-400"
                        placeholder="e.g., We provide fast and secure shipping for all orders.">{{ old('description') }}</textarea>
                </div>
                        ...
                        <div class="space-y-1 text-center">
                            ...
                            <div class="flex text-sm text-zinc-600 justify-center">
                                <label for="file-upload" class="relative cursor-pointer bg-white rounded-md font-medium text-amber-600 hover:text-amber-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-amber-500">
                                    ...
                                    <input id="file-upload" name="image" type="file" class="sr-only" required accept="image/*" onchange="previewImage(this)">
                                </label>
                ...
                <div class="pt-6 border-t border-zinc-100 flex justify-end">
                    <button type="submit"
                        class="px-8 py-3 btn-gold rounded-lg font-bold text-lg tracking-wide transform hover:-translate-y-1 transition-all shadow-lg hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500">
                        Create Feature
                    </button>
                    <a href="{{ route('admin.features.index') }}"
...
<script>
    function previewImage(input) {
        ...
        if (input.files && input.files[0]) {
            ...
            reader.onload = function(e) {
                ...
                dropZone.classList.add('border-amber-400', 'bg-amber-50/50'); // Highlight active drop zone
            }
    ...
    function removeImage() {
        ...
        dropZone.classList.remove('border-amber-400', 'bg-amber-50/50'); // Remove highlight
    }

    // Drag and Drop Logic (Optional polish)
    const dropZone = document.getElementById('drop-zone');
