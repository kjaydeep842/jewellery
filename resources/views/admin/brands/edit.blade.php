...
<h1 class="text-3xl font-premium font-bold mb-6 text-zinc-900">Edit Brand: {{ $brand->name }}</h1>
...
<div class="bg-white p-8 rounded-xl shadow-lg border border-zinc-100 animate-enter">
    ...
    <form action="{{ route('admin.brands.update', $brand->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-6">
            ...
            @if($brand->image)
            <div class="mb-3">
                <img src="{{ Storage::url($brand->image) }}" class="w-32 h-32 object-contain border rounded-lg bg-zinc-50 p-2">
                <p class="text-xs text-zinc-500 mt-1">Current Image</p>
            </div>
            @endif
            ...
            <p class="text-xs text-zinc-500 mt-1">Upload new image to replace current one. Recommended size: 200x200px. Max: 2MB.</p>
            @error('image')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-8">
            ...
            <label class="inline-flex items-center cursor-pointer">
                <input type="checkbox" name="status" value="1" class="sr-only peer" {{ old('status', $brand->status) ? 'checked' : '' }}>
