...
<div class="bg-[#F9F5EB] min-h-screen py-20 px-4">
    ...
    <div class="max-w-5xl mx-auto space-y-6">
        @forelse($faqs as $faq)
        <div x-data="{ open: false }" class="bg-white rounded-xl shadow-sm border border-zinc-100/50 overflow-hidden transition-all duration-300" :class="{ 'shadow-md': open }">
            <button @click="open = !open" class="w-full flex items-center justify-between p-8 text-left focus:outline-none group">
                <span class="text-xl font-bold text-zinc-800 group-hover:text-[#A67C52] transition-colors" :class="{ 'text-[#A67C52]': open }">
                    {{ $faq->question }}
                </span>
                <div class="flex-shrink-0 ml-6">
                    <div class="w-10 h-10 rounded-full bg-[#F3EFE0] flex items-center justify-center text-[#A67C52] font-bold text-2xl transition-transform duration-300" :class="{ 'rotate-180': open }">
