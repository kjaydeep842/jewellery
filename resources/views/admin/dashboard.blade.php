    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-lg shadow-zinc-200/50 p-6 relative overflow-hidden group hover:-translate-y-1 transition-all duration-300 animate-enter border border-zinc-100"
            ...
            <div class="flex items-baseline mt-2 space-x-2">
                <h3 class="text-3xl font-premium font-bold text-zinc-900">₹{{ number_format($totalRevenue, 0) }}</h3>
                <span class="text-xs font-bold {{ $revenueGrowth >= 0 ? 'text-emerald-600 bg-emerald-50' : 'text-red-600 bg-red-50' }} px-2 py-0.5 rounded-full">
            ...
            <div class="mt-4 pt-4 border-t border-zinc-50">
                <div class="h-1.5 w-full bg-zinc-100 rounded-full overflow-hidden">
                    <div
                        class="h-full bg-gradient-to-r from-amber-400 to-amber-600 shadow-[0_0_10px_rgba(251,191,36,0.5)]"
                        style="width: {{ min($revenueProgress, 100) }}%">
                    </div>
                </div>
                <p class="text-xs text-zinc-400 mt-2 font-medium">Target: ₹{{ number_format($revenueTarget, 0) }}</p>
            </div>
        </div>
        ...
        <div
            ...
            <div class="flex items-center justify-between mb-4 relative z-10">
                ...
                <span class="text-xs font-bold {{ $ordersGrowth >= 0 ? 'text-emerald-600 bg-emerald-100' : 'text-red-600 bg-red-100' }} px-2.5 py-1 rounded-full">
                    {{ $ordersGrowth >= 0 ? '+' : '' }}{{ number_format($ordersGrowth, 1) }}%
                </span>
            </div>
            <h3 class="text-3xl font-bold text-zinc-800">{{ $activeOrders }}</h3>
        ...
        <div
            ...
            <div class="flex items-center justify-between mb-4 relative z-10">
                ...
                <span class="text-xs font-bold {{ $customersGrowth >= 0 ? 'text-emerald-600 bg-emerald-100' : 'text-red-600 bg-red-100' }} px-2.5 py-1 rounded-full">
                    {{ $customersGrowth >= 0 ? '+' : '' }}{{ number_format($customersGrowth, 1) }}%
                </span>
            </div>
            <h3 class="text-3xl font-bold text-zinc-800">{{ number_format($totalCustomers) }}</h3>
        ...
        <div
            ...
            <div class="flex items-center justify-between mb-4 relative z-10">
                ...
                <span class="text-xs font-bold text-rose-600 bg-rose-100 px-2.5 py-1 rounded-full">Alert</span>
            </div>
            <h3 class="text-3xl font-bold text-zinc-800">{{ $lowStockItems }}</h3>
    ...
    <div class="bg-white rounded-xl shadow-lg shadow-zinc-200/50 border border-zinc-100 p-8 animate-enter"
        ...
        <div class="overflow-x-auto">
            ...
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-zinc-50">
                    ...
                    <tr class="hover:bg-zinc-50 transition-colors">
                        <td class="px-4 py-3 font-medium text-amber-600">#{{ str_pad($order->id, 5, '0', STR_PAD_LEFT) }}</td>
                        <td class="px-4 py-3 font-medium text-zinc-900">{{ $order->user->name ?? 'Guest' }}</td>
                        <td class="px-4 py-3">
                            @if($order->items->count() > 0)
                            {{ $order->items->first()->product->name ?? 'N/A' }}
                            @if($order->items->count() > 1)
                            <span class="text-zinc-400 text-xs">+{{ $order->items->count() - 1 }} more</span>
                            @endif
                            @else
                            N/A
                            @endif
                        ...
                        <td class="px-4 py-3">
                            ...
                            $statusClass = $statusColors[$order->status] ?? 'bg-zinc-100 text-zinc-700';
                            @endphp
                            <span class="px-2 py-1 {{ $statusClass }} rounded-full text-xs font-semibold capitalize">
                                {{ $order->status }}
                            </span>
                        </td>
            ...
            <div class="text-center py-12">
                ...
                <p class="text-zinc-400 text-sm mt-1">Orders will appear here once customers start purchasing</p>
            </div>
            @endif
        </div>
    </div>
