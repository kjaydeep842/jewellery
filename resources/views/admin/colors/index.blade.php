...
<div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
    <h1 class="text-2xl sm:text-3xl font-premium font-bold text-zinc-900 tracking-wide">Colors</h1>
...
<div class="bg-white border border-zinc-100 rounded-xl shadow-lg shadow-zinc-200/50 overflow-x-auto animate-enter p-4">
    <table id="colorsTable" class="w-full text-left border-collapse stripe hover">
        <thead class="bg-zinc-50 text-zinc-900 border-b border-zinc-200">
            <tr>
                ...
                <th class="p-4 font-bold font-heading text-sm uppercase tracking-wider">Code</th>
            ...
            <tr class="group hover:bg-amber-50/50 transition-colors">
                ...
                <td class="p-4">
                    <div class="flex items-center gap-2">
                        @if($color->code)
                        <div class="w-6 h-6 rounded border border-zinc-300 shadow-sm" style="background-color: {{ $color->code }}" title="{{ $color->code }}"></div>
                        @endif
                        <span class="font-bold text-zinc-800">{{ $color->name }}</span>
                    </div>
                </td>
                <td class="p-4 text-zinc-600">{{ $color->code ?? 'N/A' }}</td>
                <td class="p-4">
                    <form action="{{ route('admin.colors.toggle', $color->id) }}" method="POST" class="inline-block">
                        @csrf
                        @method('PATCH')
                        <button type="submit" class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" class="sr-only peer" {{ $color->status ? 'checked' : '' }} disabled>
                ...
                <td class="p-4">
                    <div class="flex items-center space-x-2">
                        ...
                        <form action="{{ route('admin.colors.destroy', $color->id) }}" method="POST"
                            class="inline-block" onsubmit="return confirm('Are you sure you want to delete this color?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
...
<script>
    $(document).ready(function() {
        $('#colorsTable').DataTable({
            ...
            columnDefs: [{
                orderable: false,
                targets: [3, 4] // Status and Actions columns
            }]
        });
    });
</script>
