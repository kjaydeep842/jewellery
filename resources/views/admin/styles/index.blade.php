...
@section('title', 'Unique Styles')

@section('content')

<div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
    <h1 class="text-2xl sm:text-3xl font-premium font-bold text-zinc-900 tracking-wide">Unique Styles</h1>
...
<div class="bg-white border border-zinc-100 rounded-xl shadow-lg shadow-zinc-200/50 overflow-x-auto animate-enter p-4">
    <table id="stylesTable" class="w-full text-left border-collapse stripe hover">
        <thead class="bg-zinc-50 text-zinc-900 border-b border-zinc-200">
            <tr>
                ...
                <th class="p-4 font-bold font-heading text-sm uppercase tracking-wider">Created At</th>
        ...
        <tbody class="divide-y divide-zinc-100 text-sm">
            ...
            <tr class="group hover:bg-amber-50/50 transition-colors">
                <td class="p-4 text-zinc-500">#{{ $style->id }}</td>
                <td class="p-4">
                    @if($style->image)
                    <div class="h-20 w-20 rounded-lg overflow-hidden border border-zinc-200 shadow-sm relative group/img">
                        <img src="{{ Storage::url($style->image) }}" class="w-full h-full object-cover transition-transform duration-500 group-hover/img:scale-110">
                    </div>
                    @else
                ...
                <td class="p-4">
                    <div class="flex items-center space-x-2">
                        ...
                        <form action="{{ route('admin.styles.destroy', $style->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure?');">
                            @csrf
                            @method('DELETE')
