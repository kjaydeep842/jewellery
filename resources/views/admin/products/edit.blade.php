...
<div class="container mx-auto px-4 py-6">
    ...
    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data"
        class="bg-white rounded-xl shadow-lg border border-zinc-100 p-8" x-data="{ 
                    isSubmitting: false,
                    variants: {{ json_encode($product->variants->map(function ($v) { return ['size' => $v->size ?? '', 'color' => $v->color ?? '', 'diamond_quality' => $v->diamond_quality ?? '', 'material_purity' => $v->material_purity ?? '', 'shape' => $v->shape ?? '', 'price' => $v->price ?? '', 'stock' => $v->stock_quantity]; })) }},
                    selectedCategoryId: {{ json_encode(old('category_id', $product->category_id)) }},
        ...
        <div class="mb-8 border-b border-gray-100 pb-6">
            ...
            <div class="flex gap-6 mb-6">
                ...
                @php
                    $isMainVideo = false;
                    $mainExt = strtolower(pathinfo(parse_url($product->image, PHP_URL_PATH), PATHINFO_EXTENSION));
                    $isMainVideo = in_array($mainExt, ['mp4', 'webm', 'ogg', 'mov']);
                @endphp
                <div
                    class="w-32 h-32 relative border rounded bg-gray-50 flex items-center justify-center overflow-hidden">
                    @if($isMainVideo)
                    <video src="{{ $product->image }}" class="max-h-full max-w-full object-contain" autoplay loop muted playsinline></video>
                    @else
            ...
            <div class="mb-4">
                ...
                <div class="grid grid-cols-4 md:grid-cols-6 gap-4">
                    ...
                    @php
                        $isThumbVideo = false;
                        if($img->image_path) {
                            $thumbExt = strtolower(pathinfo(parse_url($img->image_path, PHP_URL_PATH), PATHINFO_EXTENSION));
                    ...
                    <div
                        ...
                        @if($isThumbVideo)
                        <video src="{{ $img->image_path }}" class="max-h-full max-w-full object-contain" autoplay loop muted playsinline></video>
                        @else
                        <img src="{{ $img->image_path }}" class="max-h-full max-w-full">
                        @endif
                        <div
        ...
        <div class="flex justify-between items-center pt-6 border-t border-zinc-100 mt-8">
            ...
            <div class="flex gap-4">
                ...
                <button type="submit"
                    class="px-8 py-3 btn-gold hover:shadow-xl transition-all font-bold tracking-wide transform hover:-translate-y-0.5 rounded-lg shadow-md flex items-center justify-center min-w-[170px]" x-bind:disabled="isSubmitting" :class="{'opacity-75 cursor-not-allowed': isSubmitting}">
                    <span x-show="!isSubmitting">Update Product</span>
