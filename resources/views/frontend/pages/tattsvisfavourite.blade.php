...
@section('content')
<!-- Tattsvi's Favourite Banner -->
<section class="w-full bg-[#EFE4D6] py-8 md:py-10">
    <div class="max-w-[1920px] mx-auto px-4 text-center">
        <h1 class="w-auto h-[88px] mx-auto font-['Outfit'] font-medium text-[70px] leading-[88px] text-[#826230] mb-4 whitespace-nowrap">Tattsvi's Favourite</h1>
        <p class="max-w-2xl mx-auto text-sm md:text-base font-['Inter'] leading-relaxed">
            Curated with love and an eye for exceptional detail, explore the pieces that represent the heart of Tattsvi.
        </p>
    </div>
</section>
...
<main class="w-full max-w-[1920px] min-[2000px]:max-w-[2400px] mx-auto px-3 lg:px-8 py-8 font-['Outfit'] flex flex-col gap-2.5">
    ...
    <div class="w-full flex flex-col md:flex-row justify-between items-start md:items-end gap-4 self-start">
        <div class="flex flex-col gap-1">
            <div class="text-sm text-gray-500">
                <a href="{{ route('home') }}" class="hover:text-amber-600 cursor-pointer">Home</a> / <span class="text-gray-800 font-medium">Tattsvi's Favourite</span>
            </div>
    ...
    <div class="w-full flex flex-col lg:flex-row lg:items-start gap-8 mt-4">
        ...
        @include('frontend.partials.filter-sidebar', ['route' => 'page.tattsvisfavourite'])
...
<script>
    $(document).ready(function() {
        ...
        // --- 3. Mobile Filter Sidebar ---
        const $filterSidebar = $('#filter-sidebar');
        const $filterOverlay = $('#filter-overlay');

        function toggleFilterSidebar() {
            const isClosed = $filterSidebar.hasClass('-translate-x-full');
            if (isClosed) {
                // Open
        ...
        // Expose to window for inline onclicks (backward compatibility)
        window.closeFilter = toggleFilterSidebar;

        // Triggers
        $(document).on('click', '#mobile-filter-btn', toggleFilterSidebar);
        $(document).on('click', '#close-filter-btn', toggleFilterSidebar);
        $(document).on('click', '#filter-overlay', toggleFilterSidebar);
        ...
        window.updateProducts = function() {
            ...
            debounceTimer = setTimeout(function() {
                ...
                const baseUrl = "{{ route('page.tattsvisfavourite', [], false) }}";
        ...
        $(document).on('click', '.wishlist-btn', function(e) {
            ...
            // Optimistic UI Update: Toggle current state immediately
            const wasIndicatedLiked = $btn.hasClass('bg-red-500'); // Check visual state

            if (!wasIndicatedLiked) {
                $btn.removeClass('bg-white').addClass('bg-red-500');
                $icon.css('filter', 'brightness(0) invert(1)'); // White icon
            } else {
                $btn.addClass('bg-white').removeClass('bg-red-500');
                $icon.css('filter', ''); // Original icon color
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    // Optional: distinct toast message
                },
                error: function(xhr) {
                    console.error('Wishlist failed:', xhr);
                    // Revert UI on error
                    if (!wasIndicatedLiked) {
                        $btn.addClass('bg-white').removeClass('bg-red-500');
                        $icon.css('filter', '');
        ...
        $(document).on('click', '.add-to-cart-btn', function(e) {
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    ...
                    if (xhr.status === 401) {
                        window.location.href = "{{ route('login') }}"; // Redirect to login if unauthorized
                    }
                }
            });
        });
