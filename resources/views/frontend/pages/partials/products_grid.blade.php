@forelse($products as $product)
    ...
    <div class="bg-[#FAF8F1] border border-[#E8E1D5] hover:shadow-lg transition-shadow rounded-sm overflow-hidden p-2 relative group product-card"
        data-images='{{ $allImages->toJson() }}' data-current-index="0">
        <div class="w-full aspect-square flex items-center justify-center mb-3 relative rounded-sm overflow-hidden">
            ...
            @if($allImages->count() > 1)
                <button
                    class="absolute z-20 nav-prev-side hover:bg-[#C5C5C8] transition-colors cursor-pointer flex items-center justify-center"
                    style="width: 18px; height: 30px; left: 0px; top: calc(50% - 38px); background: #D7D7DA; border-radius: 0px 100px 100px 0px;">
                    <i class="fa-solid fa-chevron-left text-[10px] text-gray-500"></i>
                </button>
                <button
                    class="absolute z-20 nav-next-side hover:bg-[#C5C5C8] transition-colors cursor-pointer flex items-center justify-center"
                    style="width: 18px; height: 30px; right: 0px; top: calc(50% - 38px); background: #D7D7DA; border-radius: 100px 0px 0px 100px;">
            ...
            <div class="absolute bottom-3 inset-x-3 flex items-center justify-between gap-2 z-20">
                ...
                <!-- Add to Cart -->
                <button
                    class="add-to-cart-btn flex-grow h-9 text-white text-xs shadow-md opacity-0 group-hover:opacity-100 transition-opacity duration-300 font-['Outfit'] whitespace-nowrap"
            ...
            @if($colors->isNotEmpty())
                <div class="flex items-center gap-2 mt-2">
                    @foreach($colors as $color)
                        ...
                        @endphp
                        <div class="w-4 h-4 rounded-full {{ $bgClass }} border border-gray-300 cursor-pointer hover:ring-1 hover:ring-offset-1 hover:ring-gray-400"
                            title="{{ $color }}"></div>
                    @endforeach
                </div>
            @endif
