    ...
    if (minPriceInput && maxPriceInput) {
        function updatePriceSlider() {
            ...
            if (min > max - 1000) {
                ...
                // or just return if we want to stop strictly
            }
        ...
        // If a price checkbox is clicked, we could optionally reset the slider or just let backend priority handle it.
        // For visual consistency, let's leave the slider visually where it is, but backend will prioritize checkboxes.
        // Or better, reset slider to full range?
        document.querySelectorAll('.price-checkbox').forEach(cb => {
            cb.addEventListener('change', () => {
                // If checking a box, maybe just submit? behavior is handled by 'change' event on form which calls updateProducts
            });
        });
    }
