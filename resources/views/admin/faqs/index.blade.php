...
<div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
    <h1 class="text-2xl sm:text-3xl font-premium font-bold text-zinc-900 tracking-wide">FAQs</h1>
...
<div class="bg-white border border-zinc-100 rounded-xl shadow-lg shadow-zinc-200/50 overflow-x-auto animate-enter p-4">
    <table id="faqsTable" class="w-full text-left border-collapse stripe hover">
        <thead class="bg-zinc-50 text-zinc-900 border-b border-zinc-200">
            <tr>
                ...
                <th class="p-4 font-bold font-heading text-sm uppercase tracking-wider">Question</th>
        ...
        <tbody class="divide-y divide-zinc-100 text-sm">
            ...
            <tr class="group hover:bg-amber-50/50 transition-colors">
                <td class="p-4 text-zinc-500">#{{ $faq->id }}</td>
                <td class="p-4 font-bold text-zinc-800">{{ $faq->question }}</td>
                <td class="p-4">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" class="sr-only peer" {{ $faq->status ? 'checked' : '' }}
                            onchange="toggleFaqStatus({{ $faq->id }})">
                        <div
