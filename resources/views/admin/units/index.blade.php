...
<div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
    <h1 class="text-2xl sm:text-3xl font-premium font-bold text-zinc-900 tracking-wide">Units</h1>
...
<div class="bg-white border border-zinc-100 rounded-xl shadow-lg shadow-zinc-200/50 overflow-x-auto animate-enter p-4">
    <table id="unitsTable" class="w-full text-left border-collapse stripe hover">
        ...
        <tbody class="divide-y divide-zinc-100 text-sm">
            ...
            <tr class="group hover:bg-amber-50/50 transition-colors">
                <td class="p-4 text-zinc-500">#{{ $unit->id }}</td>
                <td class="p-4 font-bold text-zinc-800">{{ $unit->name }}</td>
                <td class="p-4 text-zinc-600">{{ $unit->code ?? 'N/A' }}</td>
                <td class="p-4">
                    <form action="{{ route('admin.units.toggle', $unit->id) }}" method="POST" class="inline-block">
                        @csrf
                        @method('PATCH')
                        <button type="submit" class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" class="sr-only peer" {{ $unit->status ? 'checked' : '' }} disabled>
                ...
                <td class="p-4">
                    <div class="flex items-center space-x-2">
                        ...
                        <form action="{{ route('admin.units.destroy', $unit->id) }}" method="POST"
                            class="inline-block" onsubmit="return confirm('Are you sure you want to delete this unit?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
