...
@section('title', 'Metals')

@section('content')

<div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
    <h1 class="text-2xl sm:text-3xl font-premium font-bold text-zinc-900 tracking-wide">Metals</h1>

    <a href="{{ route('admin.metals.create') }}"
...
<div class="bg-white border border-zinc-100 rounded-xl shadow-lg shadow-zinc-200/50 overflow-x-auto animate-enter p-4">
    <table id="metalsTable" class="w-full text-left border-collapse stripe hover">
        ...
        <tbody class="divide-y divide-zinc-100 text-sm">
            ...
            <tr class="group hover:bg-amber-50/50 transition-colors">
                <td class="p-4 text-zinc-500">#{{ $metal->id }}</td>
                <td class="p-4 font-bold text-zinc-800">{{ $metal->name }}</td>
                <td class="p-4 text-zinc-600">{{ $metal->sort_order }}</td>
                <td class="p-4">
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" class="sr-only peer toggle-status"
                            data-id="{{ $metal->id }}"
                            {{ $metal->status ? 'checked' : '' }}>
                ...
                <td class="p-4">
                    <div class="flex items-center space-x-2">
                        <a href="{{ route('admin.metals.edit', $metal->id) }}"
                        ...
                        <form action="{{ route('admin.metals.destroy', $metal->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure?');">
                            @csrf
                            @method('DELETE')
