        ...
        <table id="blogsTable" class="w-full text-left border-collapse">
            <thead class="bg-zinc-50/50 border-b border-zinc-100">
                <tr>
                    ...
                    <th class="p-5 font-bold font-heading text-xs uppercase tracking-wider text-zinc-500">Published At</th>
            ...
            <tbody class="divide-y divide-zinc-50">
                ...
                <tr class="group hover:bg-amber-50/30 transition-colors duration-200">
                    <td class="p-5 text-zinc-500 font-mono text-sm">#{{ $blog->id }}</td>
                    <td class="p-5 font-medium text-zinc-800">{{ $blog->title }}</td>
                    <td class="p-5">
                        @if($blog->image)
                    ...
                    <td class="p-5 text-zinc-600 text-sm max-w-xs truncate">{{ Str::limit(strip_tags($blog->description), 50) }}</td>
                    <td class="p-5 text-zinc-500 text-sm">{{ $blog->published_at ? $blog->published_at->format('M d, Y') : 'N/A' }}</td>
