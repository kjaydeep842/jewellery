...
<h1 class="text-3xl font-premium font-bold mb-6 text-zinc-900">Edit FAQ</h1>
    ...
    <form action="{{ route('admin.faqs.update', $faq->id) }}" method="POST">
        ...
        <div class="mb-6">
            ...
            <textarea name="answer"
                class="w-full border-zinc-300 rounded-lg shadow-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-shadow p-2.5"
                rows="6" required>{{ $faq->answer }}</textarea>
            @error('answer')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
        </div>

        <div class="mb-6">
            <label class="relative inline-flex items-center cursor-pointer group">
                <input type="checkbox" name="status" value="1" class="sr-only peer" {{ $faq->status ? 'checked' : '' }}>
