...
@section('title', 'Metal Colors')

@section('content')

<div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
    <h1 class="text-2xl sm:text-3xl font-premium font-bold text-zinc-900 tracking-wide">Metal Colors</h1>

    <a href="{{ route('admin.metal_colors.create') }}"
...
<div class="bg-white border border-zinc-100 rounded-xl shadow-lg shadow-zinc-200/50 overflow-x-auto animate-enter p-4">
    <table id="metalColorsTable" class="w-full text-left border-collapse stripe hover">
        <thead class="bg-zinc-50 text-zinc-900 border-b border-zinc-200">
            <tr>
                ...
                <th class="p-4 font-bold font-heading text-sm uppercase tracking-wider">Color Code</th>
                <th class="p-4 font-bold font-heading text-sm uppercase tracking-wider">Preview</th>
        ...
        <tbody class="divide-y divide-zinc-100 text-sm">
            @foreach($metal_colors as $color)
            <tr class="group hover:bg-amber-50/50 transition-colors">
                <td class="p-4 text-zinc-500">#{{ $color->id }}</td>
                <td class="p-4 font-bold text-zinc-800">{{ $color->name }}</td>
                <td class="p-4 text-zinc-600">{{ $color->color_code }}</td>
                <td class="p-4">
                    <div class="w-8 h-8 rounded-full border border-gray-200 shadow-sm" style="background-color: {{ $color->color_code }};" title="{{ $color->color_code }}"></div>
                </td>
                <td class="p-4 text-zinc-600">{{ $color->sort_order }}</td>
                <td class="p-4">
                ...
                <td class="p-4">
                    <div class="flex items-center space-x-2">
                        <a href="{{ route('admin.metal_colors.edit', $color->id) }}"
                        ...
                        <form action="{{ route('admin.metal_colors.destroy', $color->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure?');">
                            @csrf
                            @method('DELETE')
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('change', '.toggle-status', function() {
            ...
            fetch(`/admin/metal_colors/${id}/toggle`, {
                    method: "PATCH",
                    headers: {
