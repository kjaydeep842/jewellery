...
@section('title', 'Edit Navigation Menu')
...
<div class="mb-6">
    ...
    <h1 class="text-3xl font-premium font-bold text-zinc-900 tracking-wide">Edit Navigation Menu</h1>
</div>
...
<div class="bg-white border border-zinc-100 rounded-2xl shadow-xl shadow-zinc-200/50 p-8 max-w-2xl animate-enter">
    <form action="{{ route('admin.navigation_menus.update', $navigation_menu->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="space-y-6">
            <div>
                <label for="title" class="block text-sm font-bold text-zinc-700 mb-2 uppercase tracking-wider">Title</label>
                <input type="text" name="title" id="title" value="{{ old('title', $navigation_menu->title) }}" required
            ...
            <div>
                ...
                <input type="text" name="route_name" id="route_name" value="{{ old('route_name', $navigation_menu->route_name) }}"
                    class="w-full px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all outline-none"
            ...
            <div>
                ...
                <input type="text" name="url" id="url" value="{{ old('url', $navigation_menu->url) }}"
                    class="w-full px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all outline-none"
            ...
            <div>
                ...
                <input type="number" name="order" id="order" value="{{ old('order', $navigation_menu->order) }}" required
                    class="w-full px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all outline-none">
            </div>
            ...
            <div>
                ...
                <select name="status" id="status" required
                    class="w-full px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all outline-none">
                    <option value="active" {{ old('status', $navigation_menu->status) == 'active' ? 'selected' : '' }}>Active</option>
                    <option value="inactive" {{ old('status', $navigation_menu->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                </select>
            </div>
