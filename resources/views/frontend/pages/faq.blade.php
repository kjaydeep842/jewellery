...
@section('title', 'Frequently Asked Questions - Tattsvi')
...
@section('content')
    ...
    <div class="max-w-[1600px] min-[2000px]:max-w-[2400px] mx-auto px-6 py-4">
        <div class="flex items-center gap-2 text-sm text-[#5C5C5C] font-['Outfit']">
            ...
            <span>/</span>
            <span class="font-['Outfit'] font-medium truncate">Frequently Asked Questions</span>
        </div>
    </div>
        ...
        <div class="max-w-[1374px] mx-auto flex flex-col gap-[10px]">
            @forelse($faqs as $index => $faq)
                <div class="w-full bg-white rounded-[14px] border border-[#EFE4CD] overflow-hidden">
                    <button
                        ...
                        onclick="toggleFAQAccordion('faq-{{ $index }}', this)">
                        <span class="font-['Outfit'] font-semibold text-[#5C4522] text-sm md:text-[22px] leading-none">
                            {{ $faq->question }}
                        </span>
                        <div
                            ...
                            <i class="fa-solid {{ $index === 0 ? 'fa-minus' : 'fa-plus' }} text-[#1A1A1A] text-[10px] md:text-xs"></i>
                        </div>
                    </button>
                    <div id="faq-{{ $index }}" class="{{ $index === 0 ? '' : 'hidden' }} bg-[#FBF9F3] px-[20px] md:px-[30px] py-[12px] transition-all duration-300">
            ...
            @empty
                <div class="text-center py-10">
                    <p class="font-['Outfit'] text-[#5C5C5C]">No frequently asked questions found.</p>
                </div>
            @endforelse
        </div>
    </main>
