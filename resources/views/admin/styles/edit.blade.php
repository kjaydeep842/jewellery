...
@section('title', 'Edit Unique Style')

@section('content')

<h1 class="text-3xl font-premium font-bold mb-6 text-zinc-900">Edit Unique Style</h1>
...
<div class="bg-white p-8 rounded-xl shadow-lg border border-zinc-100 animate-enter">
    <form action="{{ route('admin.styles.update', $style->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            @if($style->image)
            <div class="mb-2 h-32 w-32 rounded-lg overflow-hidden border border-zinc-200 shadow-sm">
                <img src="{{ Storage::url($style->image) }}" class="w-full h-full object-cover">
            </div>
            @else
            <p class="text-zinc-400 italic">No image uploaded</p>
            @endif
        </div>

        <div class="mb-4">
            <label class="font-bold text-zinc-700 mb-2 block font-heading">New Image (Optional)</label>
        ...
        <div class="mb-4">
            <label class="inline-flex items-center cursor-pointer">
                <input type="checkbox" name="status" value="1" class="sr-only peer" {{ $style->status ? 'checked' : '' }}>
                <div
